	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if bares {
			...
			for _, entry := range entries {
				...
				localstart := time.Now()
				totalRepos++
				fmt.Fprintf(color.Output, "starting processing on %v\n", entryName)
				if err := ripper.Rip(ctx, filepath.Join(args[0], entryName), results); err != nil {
					if err == ripsrc.ErrNoHeadCommit {
						fmt.Fprintf(color.Output, "skipping dir, it's an empty or invalid repo %v\n", entryName)
					} else {
						fmt.Println(err)
						os.Exit(1)
					}
