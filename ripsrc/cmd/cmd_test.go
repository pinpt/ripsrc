...
func TestCmdBothOutput(t *testing.T) {
	tmpfile, err := ioutil.TempFile("", "TestStreamingOutput")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(tmpfile.Name())
	...
	p := NewCmdOptions(Options{Buffered: true, Streaming: true}, "./test/stream", tmpfile.Name())
	p.Start()
...
func TestCmdOnlyStreamingOutput(t *testing.T) {
	...
	//   stderr 2
	// Where each is printed on stdout and stderr as indicated.
	p := NewCmdOptions(Options{Buffered: false, Streaming: true}, "./test/stream", tmpfile.Name())
	p.Start()
	time.Sleep(250 * time.Millisecond) // give test/stream a moment to print something
...
func TestStreamingErrLineBufferOverflow1(t *testing.T) {
	...
	longLine := make([]byte, 3+DEFAULT_LINE_BUFFER_SIZE+2) // "bc\nAAA...zz"
	longLine[0] = 'b'
	longLine[1] = 'c'
	longLine[2] = '\n'
	for i := 3; i < DEFAULT_LINE_BUFFER_SIZE; i++ {
		longLine[i] = 'A'
	}
	// These 2 chars cause ErrLineBufferOverflow:
	longLine[DEFAULT_LINE_BUFFER_SIZE] = 'z'
	longLine[DEFAULT_LINE_BUFFER_SIZE+1] = 'z'
	...
	switch err.(type) {
	case ErrLineBufferOverflow:
		lbo := err.(ErrLineBufferOverflow)
		if lbo.BufferSize != DEFAULT_LINE_BUFFER_SIZE {
			t.Errorf("ErrLineBufferOverflow.BufferSize = %d, expected %d", lbo.BufferSize, DEFAULT_LINE_BUFFER_SIZE)
		}
		if lbo.BufferFree != DEFAULT_LINE_BUFFER_SIZE {
			t.Errorf("ErrLineBufferOverflow.BufferFree = %d, expected %d", lbo.BufferFree, DEFAULT_LINE_BUFFER_SIZE)
		}
	...
	default:
		t.Errorf("got err '%v', expected ErrLineBufferOverflow", err)
	}
...
func TestStreamingErrLineBufferOverflow2(t *testing.T) {
	...
	longLine := make([]byte, DEFAULT_LINE_BUFFER_SIZE)
	for i := 0; i < DEFAULT_LINE_BUFFER_SIZE; i++ {
		longLine[i] = 'X'
	}
	n, err = out.Write(longLine)
	if n != 0 {
	...
	case ErrLineBufferOverflow:
		...
		if lbo.BufferFree != DEFAULT_LINE_BUFFER_SIZE-3 {
			t.Errorf("ErrLineBufferOverflow.BufferFree = %d, expected %d", lbo.BufferFree, DEFAULT_LINE_BUFFER_SIZE)
		}
...
func TestStreamingSetLineBufferSize(t *testing.T) {
	...
	longLine[DEFAULT_LINE_BUFFER_SIZE] = 'z'
	longLine[DEFAULT_LINE_BUFFER_SIZE+1] = '\n'

	lines := make(chan string, 5)
	out := NewOutputStream(lines)
	out.SetLineBufferSize(DEFAULT_LINE_BUFFER_SIZE * 2)
	...
	expectLine := string(longLine[3 : DEFAULT_LINE_BUFFER_SIZE+1]) // not newline
	if gotLine != expectLine {
		t.Errorf("got line: '%s', expected '%s'", gotLine, expectLine)
	}
}
...
func TestDone(t *testing.T) {
	...
	p := NewCmd("./test/count-and-sleep", "3", "1")
	statusChan := p.Start()
